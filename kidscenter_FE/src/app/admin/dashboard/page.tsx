...
export default function AdminDashboard() {
  ...
  // Filter States Jasa
  const [filterJasaStatus, setFilterJasaStatus] = useState("Semua");
  const [filterJasaDate, setFilterJasaDate] = useState("");
  
  const [isLoading, setIsLoading] = useState(true);

  useEffect(() => {
    if (isAuthorized) {
      // Kita panggil semua fetch awal sekaligus agar loader bisa hilang saat semuanya selesai
      Promise.all([
        fetchProjects(),
        fetchProductsData(),
        fetchOrdersData()
  ...
  // Real Data: Pesanan Masuk (Catalog)
  const [orders, setOrders] = useState<any[]>([]);
  const [selectedOrder, setSelectedOrder] = useState<any>(null);

  // Filter States Orders
  const [filterOrderStatus, setFilterOrderStatus] = useState("Semua");
  ...
  const formatToIndoDate = (dateString: string) => {
    if (!dateString) return "";
    const [year, month, day] = dateString.split("-");
    const months = ["Jan", "Feb", "Mar", "Apr", "Mei", "Jun", "Jul", "Agt", "Sep", "Okt", "Nov", "Des"];
    const monthName = months[parseInt(month, 10) - 1];
    return `${day} ${monthName} ${year}`; // Format ex: "12 Agt 2026"
  };

  // --- Filter Logic Jasa ---
  const handleFilterJasaChange = (key: string, value: string) => {
    setIsLoading(true);
    if (key === "status") setFilterJasaStatus(value);
    if (key === "date") setFilterJasaDate(value);
    setTimeout(() => setIsLoading(false), 500);
  };

  const filteredJasaProjects = jasaProjects.filter(p => {
    let matchStatus = true;
    let matchDate = true;
    if (filterJasaStatus !== "Semua") {
      matchStatus = p.status === filterJasaStatus;
    }
    if (filterJasaDate) {
      matchDate = p.date === formatToIndoDate(filterJasaDate);
    }
    return matchStatus && matchDate;
  });

  // --- Filter Logic Orders ---
  const handleFilterOrderChange = (key: string, value: string) => {
    setIsLoading(true);
    if (key === "status") setFilterOrderStatus(value);
    if (key === "date") setFilterOrderDate(value);
    setTimeout(() => setIsLoading(false), 500);
  };
  ...
  const filteredOrders = orders.filter(o => {
    let matchStatus = true;
    let matchDate = true;
    if (filterOrderStatus !== "Semua") {
      matchStatus = o.status === filterOrderStatus;
    }
    if (filterOrderDate) {
      matchDate = o.date === formatToIndoDate(filterOrderDate);
    }
    return matchStatus && matchDate;
  });
            ...
            <div>
              <div className="admin-header" style={{ flexDirection: 'column', alignItems: 'flex-start', gap: '1rem' }}>
                ...
                <div style={{ display: 'flex', gap: '1rem', width: '100%', flexWrap: 'wrap' }}>
                  <div className="admin-form-group" style={{ flex: 1, minWidth: '200px', margin: 0 }}>
                    <label style={{ fontSize: '0.85rem' }}>Filter Status</label>
                    <select className="admin-form-select" value={filterJasaStatus} onChange={(e) => handleFilterJasaChange("status", e.target.value)}>
                  ...
                  <div className="admin-form-group" style={{ flex: 1, minWidth: '200px', margin: 0 }}>
                    <label style={{ fontSize: '0.85rem' }}>Filter Tanggal</label>
                    <input type="date" className="admin-form-input" value={filterJasaDate} onChange={(e) => handleFilterJasaChange("date", e.target.value)} />
                  </div>
                </div>
              </div>
              <div className="admin-table-container">
                <table className="admin-table">
                  ...
                  <tbody>
                    {filteredJasaProjects.length > 0 ? (
                      filteredJasaProjects.map(p => (
                        <tr key={p.id}>
                          <td>{p.id}</td>
                          <td>{p.client}</td>
                          <td>{p.title}</td>
                      ...
                      <tr>
                        <td colSpan={6} style={{ textAlign: "center", padding: "2rem" }}>Tidak ada data yang sesuai filter.</td>
                      </tr>
                    )}
                  </tbody>
                </table>
            ...
            <div>
              <div className="admin-header" style={{ flexDirection: 'column', alignItems: 'flex-start', gap: '1rem' }}>
                ...
                <div style={{ display: 'flex', gap: '1rem', width: '100%', flexWrap: 'wrap' }}>
                  <div className="admin-form-group" style={{ flex: 1, minWidth: '200px', margin: 0 }}>
                    <label style={{ fontSize: '0.85rem' }}>Filter Status</label>
                    <select className="admin-form-select" value={filterOrderStatus} onChange={(e) => handleFilterOrderChange("status", e.target.value)}>
                      <option value="Semua">Semua Status</option>
                  ...
                  <div className="admin-form-group" style={{ flex: 1, minWidth: '200px', margin: 0 }}>
                    <label style={{ fontSize: '0.85rem' }}>Filter Tanggal</label>
                    <input type="date" className="admin-form-input" value={filterOrderDate} onChange={(e) => handleFilterOrderChange("date", e.target.value)} />
                  </div>
                </div>
              </div>
              <div className="admin-table-container">
